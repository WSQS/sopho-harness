...
class TaskContext(BaseModel):
    relevant_files: list[str] = Field(
        description="The files most relevant to the current task."
    )
    relevant_symbols: list[str] = Field(
        description="The functions, classes, methods, or other symbols most relevant to the current task."
    )
    existing_patterns: list[str] = Field(
        description="Existing implementation patterns, conventions, or abstractions that the task should follow."
    )
    possible_change_points: list[str] = Field(
        description="The most likely files, functions, or modules where changes may be needed."
    )
    local_constraints: list[str] = Field(
        description="Task-specific constraints or invariants that should be preserved in the affected area of the codebase."
    )
    open_questions: list[str] = Field(
        description="Important local uncertainties that may affect planning or implementation."
    )
    extracts: str = Field(
        description="Short supporting excerpts or evidence from relevant files that justify the local context summary."
    )
