...
def write_patch(content: str) -> str:
    ...
    with NamedTemporaryFile(
        "w", encoding="utf-8", delete=False, suffix=".patch"
    ) as tmp:
        tmp.write(content)
...
def build_python_tools(config: PythonConfig) -> list[Tool]:
    ...
    def run_python_tests() -> str:
        command = (
            [entrypoint, "run", "pytest"]
            if entrypoint == "uv"
            else [entrypoint, "pytest"]
        )
    ...
    ruff_check_command = (
        [entrypoint, "run", "ruff", "--version"]
        if entrypoint == "uv"
        else [entrypoint, "ruff", "--version"]
    )
    ...
    if ruff_check_result.returncode != 0:
        print(
            "[tools] ruff is not available; skipping run_python_lint tool. Please install ruff manually."
        )
    ...
    def run_python_lint() -> str:
        command = (
            [entrypoint, "run", "ruff", "check", "."]
            if entrypoint == "uv"
            else [entrypoint, "ruff", "check", "."]
        )
        result = subprocess.run(command, capture_output=True, text=True, cwd=Path.cwd())
        output = result.stdout or result.stderr
